...
export const AboutSection = () => (
    <section id="about" className="section-shell relative">
        <SideLabel side="left">Nº 02 — {t('about.sideLabel')}</SideLabel>
        <Container className="relative">
            <EditorialRule index="II." page="002 / 008">{t('about.eyebrowRule')}</EditorialRule>
            ...
            <div className="mt-12 grid gap-x-20 gap-y-12 lg:grid-cols-[1.05fr_1fr] lg:items-start">
                ...
                >
                    <MetaLabel>{t('about.metaLabel')}</MetaLabel>
                    <h2 className="display mt-7 max-w-[12ch] text-[clamp(4rem,6vw,7.4rem)]">
                        {t('about.titleLead')}{' '}
                        <em className="text-[var(--accent-text)]">{t('about.titleEmphasis')}</em>{t('about.titleTail')}
                    </h2>
                    ...
                    <dl className="mt-14 border-t border-[var(--line-strong)]">
                        {principleIndexes.map((index) => (
                            ...
                            >
                                ...
                                <dd className="text-[1.4rem] leading-relaxed text-[var(--text-muted)]">{t(`about.principles.${index}.text`)}</dd>
                            </motion.div>
                        ))}
                    </dl>
                </motion.div>
